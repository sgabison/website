...
class Resource extends \Object\Concrete\Resource {
	...
	public function getStatistics (  $locationId, $start= null, $end = null ){
		...
		$sql = sprintf(" SELECT count(o.oo_id) as nbre, sum(o.partysize) as couverts,
				min(o.start) as date_start, max(o.end) as date_end,  YEAR(FROM_UNIXTIME(o.start)) as y,  MONTH(FROM_UNIXTIME(o.start)) as m,  DAY(FROM_UNIXTIME(o.start)) as d,o.location__id as location_id, o.serving__id as serving_id, s.title as serving_name
				FROM object_query_11 o join object_query_13 s on s.oo_id = o.serving__id 
				WHERE  o.`start`>= '%d' AND o.`start`<= '%d' AND o.`location__id` = '%d' and (lower(o.status) <>'cancelled' or o.status is null) ",  $start->getTimestamp(), $end->getTimestamp(), $locationId);
