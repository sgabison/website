 var SVExamples = function() {
    ...
    var checkNote = function() {
        ...
        if ($('.form-note .note-title').val() !== "" || $note.code() !== $note.attr("placeholder")) {
            bootbox.confirm(
			"You did not save note, are you sure to cancel?", 
			function(result) {
					if (result) {
    ...
    var runNoteFormValidation = function() {
        ...
        formNote.validate({
            ...
            submitHandler: function(form) {
                ...
                noteToSave.title = $('.form-note .note-title').val(), 
				noteToSave.shortContent = jQuery.truncate($note.code(), {
                    length: 200
                }), 
				noteToSave.content = $note.code(), 
				noteToSave.author = "Peter Clark", 
				noteToSave.avatar = "assets/images/avatar-1-small.jpg", 
